...
/**
 * Builds a catalog API client that attaches the login token per request.
 *
 ...
 *
 * @param {string} endpoint Path relative to the API, like 'connectors/'
 * @returns Tuple of success flag and data or error body
 * @example
 * const [worked, connectors] = await getFromApi('connectors/');
 * if (!worked) return showEmpty();
 */
